...
#recommended first guess based on monte carlo analysis: 'reals'
not_present_string = 'ralth'
perfect_placements = [('s',1), ('o', 3), ('n', 4), ('e', 5)]
imperfect_placements = []
wordl_inputs = ['reals', 'stone']

word_list = wordl_filters(word_list, not_present_string, perfect_placements, imperfect_placements)
...
while True:
    ...
    next_guess = random.sample(word_list, 1)[0]
    not_present_string, perfect_placements, imperfect_placements =  get_wordl_placement_information(wordl_inputs + [next_guess], hidden_word)
    ...
    update_statistics(stats_dictionary, next_guess, FOM)
    if ix % 10000 == 0:
        list_of_list_of_statistics = [[key,stats_dictionary[key][0], stats_dictionary[key][1]] for key in stats_dictionary]
        with open('next_word_output_statistics.csv', 'w', newline='') as f:
            writer = csv.writer(f)
